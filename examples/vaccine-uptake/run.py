...
for i in range(max_hh_size):
    ax.plot(uptake_by_time_by_size[i, vstart:], label = 'HH size'+ str(i+1))
    ax.set_ylabel('Uptake')
    ax.set_xlabel('Week')
ax.plot(uptake_by_time[vstart:], ls = ":", color = "k", label = 'Overall')
ax.legend()
fig_uptake.show()
...
for hh_size in range(1, max_hh_size + 1):
    where_size = where(hh_size_by_uptake_column == hh_size)[0]
    udf = uptake_df.iloc[vstart:, 1 + where_size]
...
axs[2, 1].legend(bbox_to_anchor=(1.1, 1.05),
                 title = "V")
axs[0, 0].set_ylabel('No. HHs')
axs[1, 0].set_ylabel('No. HHs')
axs[2, 0].set_ylabel('No. HHs')
...
def get_eq_rstar_at_time(t):
    ...
    if uptake==0:
        ...
        # Work out household size distribution
        composition_list_t = deepcopy(composition_list)
        ...
        remaining_dose = uptake * ave_hh_size
        while remaining_dose > 0:
            # print("c=",current_priority)
            # print("r=",remaining_dose)
            # print("d=",composition_dist[current_priority])
            dose_to_use = composition_dist[current_priority]
            if remaining_dose >= dose_to_use:
                if composition_dist[current_priority] > 0:
                    # Use dose
                    remaining_dose -= dose_to_use
                    destination = vmap[current_priority]
...
def uptake_from_cd(cd):
    return cd.dot(v_by_comp)/(cd.dot(size_by_comp))


uptake_from_eq_results = [r[4] for r in results_eq[vstart:]]
uptake_err = abs(array(uptake_from_eq_results) - array(uptake_by_time[33:])) / array(uptake_by_time[33:])
print("Max difference in uptake =", uptake_err.max())
uptake_eq_direct = [uptake_from_cd(r[0]) for r in results_eq[vstart:]]
...
def get_hh_vacc_rstar_at_time(t):
    ...
    if uptake==0:
        ...
        hh_size_dist_t = array([sum(uptake_dist[where(hh_size_by_comp == hhs)[0]]) for
               hhs in range(1, max(hh_size_by_comp) + 1)])
        # hh_size_dist_t = starting_composition_dist[where(starting_composition_dist>0)[0]]
        hh_size_dist_t = hh_size_dist_t / sum(hh_size_dist_t)
        ...
        composition_dist[all_u] = hh_size_dist_t - composition_dist[all_v]
...
uptake_err = abs(array(uptake_from_hh_results[33:]) - array(uptake_by_time[33:])) / array(uptake_by_time[33:])
print("Max difference in uptake =", uptake_err.max())
uptake_hh_direct = [uptake_from_cd(r[0]) for r in results_hh[33:]]
uptake_err = abs(array(uptake_hh_direct) - array(uptake_by_time[33:])) / array(uptake_by_time[33:])
print("Max difference in uptake =", uptake_err.max())
...
ax_rstar.plot(rstar_by_time, obs_plotstyle, label = 'Observed')
# ax_rstar.plot(rstar_irv_by_time, irv_plotstyle, label = 'IRV')
ax_rstar.plot(rstar_eq_by_time, eq_plotstyle, label = 'Equalising')
ax_rstar.plot(rstar_hh_by_time, hh_plotstyle, label = 'Household')
ax_rstar.set_ylim([0, 14])
ax_rstar.set_xlabel("Week")
ax_rstar.set_ylabel("$R_*$")
ax_rstar.set_box_aspect(1)

ax_threshold.plot(100 * threshold_by_time, obs_plotstyle, label = 'Observed')
# ax_threshold.plot(100 * threshold_irv, irv_plotstyle, label = 'IRV')
ax_threshold.plot(100 * threshold_eq, eq_plotstyle, label = 'Equalising')
ax_threshold.plot(100 * threshold_hh, hh_plotstyle, label = 'Household')
...
def get_obs_dist_by_t(t):
    obs_dist = uptake_df.drop('week', axis=1).iloc[t].to_numpy()
    obs_dist = obs_dist / sum(obs_dist)
    return obs_dist


irv_kl_div = [kl_div(get_obs_dist_by_t(t), results_irv[t][0]).sum() for t in range(vstart, n_wks)]
eq_kl_div = [kl_div(get_obs_dist_by_t(t), (1e-9 + results_eq[t][0]) / (1 + 1e-9)).sum() for t in range(vstart, n_wks)]
hh_kl_div = [kl_div(get_obs_dist_by_t(t), (1e-9 + results_hh[t][0]) / (1 + 1e-9)).sum() for t in range(vstart, n_wks)]

fig_kl, ax = subplots(1,
                      1)
# ax.plot(irv_kl_div, irv_plotstyle, label='IRV')
ax.plot(eq_kl_div, eq_plotstyle, label='Equalising')
ax.plot(hh_kl_div, hh_plotstyle, label='Household')
ax.legend()
fig_kl.show()
...
# Total variation distance is better!


irv_tvd_div = [abs(get_obs_dist_by_t(t) - results_irv[t][0]).sum() for t in range(vstart, n_wks)]
eq_tvd_div = [abs(get_obs_dist_by_t(t) - results_eq[t][0]).max() for t in range(vstart, n_wks)]
hh_tvd_div = [abs(get_obs_dist_by_t(t) - results_hh[t][0]).max() for t in range(vstart, n_wks)]

fig_tvd, ax = subplots(1,
                      1)
# ax.plot(irv_tvd_div, irv_plotstyle, label='IRV')
ax.plot(eq_tvd_div, eq_plotstyle, label='Equalising')
ax.plot(hh_tvd_div, hh_plotstyle, label='Household')
ax.set_xlabel('Week')
...
if save_figs:
    fname = 'plots/vaccine-uptake/tvd_divergence.png'
    fig_tvd.savefig(fname, bbox_inches='tight', dpi=300)

# Proportion of households with zero doses under observed, IRV, and household vaccination
def get_zero_dose_prop(cd, hh_size):
    ...
    p_hh_size = sum(cd[where(hh_size_by_uptake_column==hh_size)[0]])

    return cd[zero_dose_loc][0] / p_hh_size

zero_dose_by_size = array([[get_zero_dose_prop(get_obs_dist_by_t(t), hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])

zero_dose_irv = array([[get_zero_dose_prop(results_irv[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])
zero_dose_eq = array([[get_zero_dose_prop(results_eq[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])
zero_dose_hh = array([[get_zero_dose_prop(results_hh[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])

# Proportion of households with zero unvaccinated under observed, IRV, and household vaccination
def get_full_vacc_prop(cd, hh_size):
    # Get position in composition_list of [0, hh_size]
    full_vacc_loc = where(abs(composition_list - array([0, hh_size])).max(1) < .1)[0]

    # Condition on household size
    p_hh_size = sum(cd[where(hh_size_by_uptake_column == hh_size)[0]])

    return cd[full_vacc_loc][0] / p_hh_size

full_vacc_by_size = array([[get_full_vacc_prop(get_obs_dist_by_t(t), hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])

full_vacc_irv = array([[get_full_vacc_prop(results_irv[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])
full_vacc_eq = array([[get_full_vacc_prop(results_eq[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])
full_vacc_hh = array([[get_full_vacc_prop(results_hh[t][0], hh_size) for t in range(vstart, n_wks)]
                           for hh_size in range(1, max_hh_size + 1)])


# Plots of zero/all vacc

fig_zero_full, ax = subplots(max_hh_size,
                             2,
                             figsize = (6, 10),
                             sharex=True,
                             sharey=True)
...
for i in range(max_hh_size):
    ax[i, 0].plot(zero_dose_by_size[i, :], obs_plotstyle, label = 'Observed')
    # ax[i, 0].plot(zero_dose_irv[i+1, :], irv_plotstyle, label = 'IRV')
    ax[i, 0].plot(zero_dose_eq[i, :], eq_plotstyle, label = "Equalising")
    ax[i, 0].plot(zero_dose_hh[i, :], hh_plotstyle, label = "Household")
    ax[i, 0].set_ylabel('Proportion')

    ax[i, 1].plot(full_vacc_by_size[i, :], obs_plotstyle, label='Observed')
    # ax[i, 1].plot(full_vacc_irv[i + 1, :], irv_plotstyle, label='IRV')
    ax[i, 1].plot(full_vacc_eq[i, :], eq_plotstyle, label="Equalising")
    ax[i, 1].plot(full_vacc_hh[i, :], hh_plotstyle, label="Household")
    ax[i, 1].text(180, 0.25, 'N =' + str(i+1))

ax[0, 0].set_title("Fully unvaccinated")
ax[0, 1].set_title("Fully vaccinated")
ax[max_hh_size-1, 0].set_xlabel("Week")
ax[max_hh_size-1, 1].set_xlabel("Week")
