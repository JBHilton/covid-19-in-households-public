...
# First check: does solving IVPs give you same outcome as exponential?
DOUBLING_TIME = 3
...
tspan = (0.0, 140.5)
t_eval = arange(0., 140.5, 14.)
solution = solve_ivp(rhs, tspan, H0, first_step=0.001, atol=1e-16, t_eval = t_eval)

Q1 = rhs.Q_int_inf
Q2 = rhs.Q_import
Q0 = rhs.Q_int_fixed
Q_theta = Q1.T + Q2.T + Q0.T

linear_sol = [H0]
for i in range(1, len(t_eval)):
    linear_sol += [expm((t_eval[i] -t_eval[i-1]) * Q_theta).dot(linear_sol[i-1])]
...
for i in range(n_attempt):
    ...
    t_ode += time() - t
    t = time()
    linear_sol = do_matrix_solve(t_eval, H0)
    t_matrix += time() - t

print("Ave. execution time for ODE solves:", t_ode / n_attempt)
print("Ave. execution time for matrix solves:", t_matrix / n_attempt)

# Next check: how do likelihood estimates differ between ODE solves and matrix exponentials?
