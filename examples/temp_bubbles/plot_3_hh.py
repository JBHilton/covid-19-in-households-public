...
with open('bubble_results_3_12.pkl', 'rb') as f:
    output_data = load(f)
...
ax.plot(output_data.baseline_time, output_data.baseline_E, label='E')
ax.plot(output_data.baseline_time, output_data.baseline_I, label='I')
ax.plot(output_data.baseline_time, output_data.baseline_R, label='R')
ax.legend(ncol=1, bbox_to_anchor=(1,0.50))
fig.savefig('baseline_epidemic.png', bbox_inches='tight', dpi=300)
...
lgd=['No bubbling','Bubble with single other hh for five days',
    ...
    'Bubble with new hh every day for 3 days',]
...
fig.savefig('R_by_strategy_3hh.png', bbox_inches='tight', dpi=300)
