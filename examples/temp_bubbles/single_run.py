...
class UnmergedContext:
    def __init__(self, i, density_exponent, t_start, t_end, merge_start):
        ...
        self.baseline_output = [baseline_time,
                                baseline_S,
                                baseline_E,
                                baseline_I,
                                baseline_R]
        with open('outputs/temp_bubbles/baseline_ts.pkl', 'wb') as f:
            dump(self.baseline_output, f)
...
def run_merge(
    ...
    t_0 = hstack((t_merge_0, t_postmerge_0))
    merge_S_0 = (1/2) * H_merge_0.T.dot(
        merged_population2.states[:, 0] + merged_population2.states[:, 4])/unmerged.input.ave_hh_size
    postmerge_S_0 = H_postmerge_0.T.dot(unmerged.population.states[:, 0])/unmerged.input.ave_hh_size
    S_0 = hstack((merge_S_0, postmerge_S_0))
    merge_E_0 = (1/2) * H_merge_0.T.dot(
        merged_population2.states[:, 1] + merged_population2.states[:, 5])/unmerged.input.ave_hh_size
    postmerge_E_0 = H_postmerge_0.T.dot(unmerged.population.states[:, 1])/unmerged.input.ave_hh_size
    E_0 = hstack((merge_E_0, postmerge_E_0))
    ...
    I_0 = hstack((merge_I_0, postmerge_I_0))
    ...
    R_0 = hstack((merge_R_0, postmerge_R_0))
    ...
    output_0 = [t_0,
                ...
                I_0,
                R_0]
    with open('outputs/temp_bubbles/ts_0.pkl', 'wb') as f:
        dump(output_0, f)

    t_1 = hstack((t_merge_1, t_postmerge_1))
    merge_S_1 = (1/3) * H_merge_1.T.dot(
        merged_population3.states[:, 0] + merged_population3.states[:, 4] +
        merged_population3.states[:,8])/unmerged.input.ave_hh_size
    postmerge_S_1 = H_postmerge_1.T.dot(unmerged.population.states[:, 0])/unmerged.input.ave_hh_size
    S_1 = hstack((merge_S_1, postmerge_S_1))
    merge_E_1 = (1/3) * H_merge_1.T.dot(
        merged_population3.states[:, 1] + merged_population3.states[:, 5] +
        merged_population3.states[:,9])/unmerged.input.ave_hh_size
    postmerge_E_1 = H_postmerge_1.T.dot(unmerged.population.states[:, 1])/unmerged.input.ave_hh_size
    E_1 = hstack((merge_E_1, postmerge_E_1))
    ...
    I_1 = hstack((merge_I_1, postmerge_I_1))
    ...
    R_1 = hstack((merge_R_1, postmerge_R_1))
    ...
    output_1 = [t_1,
                ...
                I_1,
                R_1]
    with open('outputs/temp_bubbles/ts_1.pkl', 'wb') as f:
        dump(output_1, f)

    t_2 = hstack((t_merge_2, t_postmerge_2))
    merge_S_2 = (1/2) * H_merge_2.T.dot(
        merged_population2.states[:, 0] + merged_population2.states[:, 4])/unmerged.input.ave_hh_size
    postmerge_S_2 = H_postmerge_2.T.dot(unmerged.population.states[:, 0])/unmerged.input.ave_hh_size
    S_2 = hstack((merge_S_2, postmerge_S_2))
    merge_E_2 = (1/2) * H_merge_2.T.dot(
        merged_population2.states[:, 1] + merged_population2.states[:, 5])/unmerged.input.ave_hh_size
    postmerge_E_2 = H_postmerge_2.T.dot(unmerged.population.states[:, 1])/unmerged.input.ave_hh_size
    E_2 = hstack((merge_E_2, postmerge_E_2))
    merge_I_2 = (1/2) * H_merge_2.T.dot(
        merged_population2.states[:, 2] + merged_population2.states[:, 6])/unmerged.input.ave_hh_size
    postmerge_I_2 = H_postmerge_2.T.dot(unmerged.population.states[:, 2])/unmerged.input.ave_hh_size
    I_2 = hstack((merge_I_2, postmerge_I_2))
    merge_R_2 = (1/2) * H_merge_2.T.dot(
        merged_population2.states[:, 3] + merged_population2.states[:, 7])/unmerged.input.ave_hh_size
    postmerge_R_2 = H_postmerge_2.T.dot(unmerged.population.states[:, 3])/unmerged.input.ave_hh_size
    R_2 = hstack((merge_R_2, postmerge_R_2))
    ...
    output_2 = [t_2,
                ...
                I_2,
                R_2]
    with open('outputs/temp_bubbles/ts_2.pkl', 'wb') as f:
        dump(output_2, f)

    t_3 = hstack((t_merge_3, t_postmerge_3))
    merge_S_3 = (1/2) * H_merge_3.T.dot(
        merged_population2.states[:, 0] + merged_population2.states[:, 4])/unmerged.input.ave_hh_size
    postmerge_S_3 = H_postmerge_3.T.dot(unmerged.population.states[:, 0])/unmerged.input.ave_hh_size
    S_3 = hstack((merge_S_3, postmerge_S_3))
    merge_E_3 = (1/2) * H_merge_3.T.dot(
        merged_population2.states[:, 1] + merged_population2.states[:, 5])/unmerged.input.ave_hh_size
    postmerge_E_3 = H_postmerge_3.T.dot(unmerged.population.states[:, 1])/unmerged.input.ave_hh_size
    E_3 = hstack((merge_E_3, postmerge_E_3))
    merge_I_3 = (1/2) * H_merge_3.T.dot(
        merged_population2.states[:, 2] + merged_population2.states[:, 6])/unmerged.input.ave_hh_size
    postmerge_I_3 = H_postmerge_3.T.dot(unmerged.population.states[:, 2])/unmerged.input.ave_hh_size
    I_3 = hstack((merge_I_3, postmerge_I_3))
    merge_R_3 = (1/2) * H_merge_3.T.dot(
        merged_population2.states[:, 3] + merged_population2.states[:, 7])/unmerged.input.ave_hh_size
    postmerge_R_3 = H_postmerge_3.T.dot(unmerged.population.states[:, 3])/unmerged.input.ave_hh_size
    R_3 = hstack((merge_R_3, postmerge_R_3))
    ...
    output_3 = [t_3,
                ...
                I_3,
                R_3]
    with open('outputs/temp_bubbles/ts_3.pkl', 'wb') as f:
        dump(output_3, f)

    t_4 = hstack((t_merge_4, t_postmerge_4))
    merge_S_4 = (1/2) * H_merge_4.T.dot(
        merged_population2.states[:, 0] + merged_population2.states[:, 4])/unmerged.input.ave_hh_size
    postmerge_S_4 = H_postmerge_4.T.dot(unmerged.population.states[:, 0])/unmerged.input.ave_hh_size
    S_4 = hstack((merge_S_4, postmerge_S_4))
    merge_E_4 = (1/2) * H_merge_4.T.dot(
        merged_population2.states[:, 1] + merged_population2.states[:, 5])/unmerged.input.ave_hh_size
    postmerge_E_4 = H_postmerge_4.T.dot(unmerged.population.states[:, 1])/unmerged.input.ave_hh_size
    E_4 = hstack((merge_E_4, postmerge_E_4))
    merge_I_4 = (1/2) * H_merge_4.T.dot(
        merged_population2.states[:, 2] + merged_population2.states[:, 6])/unmerged.input.ave_hh_size
    postmerge_I_4 = H_postmerge_4.T.dot(unmerged.population.states[:, 2])/unmerged.input.ave_hh_size
    I_4 = hstack((merge_I_4, postmerge_I_4))
    merge_R_4 = (1/2) * H_merge_4.T.dot(
        merged_population2.states[:, 3] + merged_population2.states[:, 7])/unmerged.input.ave_hh_size
    postmerge_R_4 = H_postmerge_4.T.dot(unmerged.population.states[:, 3])/unmerged.input.ave_hh_size
    R_4 = hstack((merge_R_4, postmerge_R_4))
    ...
    output_4 = [t_4,
                ...
                I_4,
                R_4]
    with open('outputs/temp_bubbles/ts_4.pkl', 'wb') as f:
        dump(output_4, f)
...
def main():
    ...
    unmerged_exponents = 0.5
    merged_exponents = 0.5
    ...
    if isfile('outputs/temp_bubbles/threewise_merge_comps.pkl') is True:
        ...
        print('merged_comp_dist_2 sums to',merged_comp_dist_2.sum())
        print('merged_comp_dist_3 sums to',merged_comp_dist_3.sum())
    ...
    if isfile('outputs/temp_bubbles/unmgerged_context.pkl') is True:
        with open('outputs/temp_bubbles/unmgerged_context.pkl', 'rb') as f:
            unmerged_results = load(f)
    else:
        params = (0, unmerged_exponents, t0, t_end, merge_start)
        unmerged_results = create_unmerged_context(params)
        with open('outputs/temp_bubbles/unmerged_context.pkl', 'wb') as f:
            dump(unmerged_results, f)

    if isfile('outputs/temp_bubbles/merged_populations.pkl') is True:
        with open('outputs/temp_bubbles/merged_populations.pkl', 'rb') as f:
            merged_populations = load(f)
    else:
        params = (
            merged_exponents,
            merged_comp_list_2,
        ...
        merged_populations = create_merged_systems(params)
        with open('outputs/temp_bubbles/merged_populations.pkl', 'wb') as f:
            dump(merged_populations, f)

    state_match_2 = match_merged_states_to_unmerged(
        unmerged_results.population,
        merged_populations.merged_population2,
        pairings_2,
        2,
        NO_COMPARTMENTS)
    state_match_3 = match_merged_states_to_unmerged(
        unmerged_results.population,
        merged_populations.merged_population3,
        pairings_3,
        3,
        NO_COMPARTMENTS)
    ...
    results = unpack_paramas_and_run_merge(params)
