...
from examples.temp_bubbles.common import ( DataObject,SINGLE_AGE_CLASS_SEIR_SPEC, SingleClassSEIRInput,
        MergedSEIRInput, demerged_initial_condition,
        ...
        match_merged_states_to_unmerged, initialise_merged_system,
        build_mixed_compositions_threewise, initialise_merged_system_threewise)
...
def simulate_merge3(duration_list,switches,premerge_H0,t0,t_end):
    for switch in range(switches+1):
        # print('Initialising merge number',switch,'...')
        H0 = initialise_merged_system_threewise(premerge_H0,
                                    unmerged_population,
                                    merged_population3,
                                    state_match)
        tspan = (t0, t0 + duration_list[switch])
...
for i in range(unmerged_expo_len):
    ...
    with open(filename_stem + '.pkl', 'wb') as f:
        dump((unmerged_population,baseline_H, baseline_time, baseline_S, baseline_E, baseline_I, baseline_R),
         f)
    ...
    for j in range(merged_expo_len):
        ...
        merged_input3 = MergedSEIRInput(SINGLE_AGE_CLASS_SEIR_SPEC,3,1)
        ...
        if i==0 and j==0: # We only need to generate this thing once, it is very time consuming
            state_match = match_merged_states_to_unmerged(unmerged_population,
                                                 merged_population3,
                                                 pairings_3,
                                                 hh_to_merge,
                                                 4)
